  ...
  return (
    <form action={action} className="surface rounded-2xl p-6 sm:p-8">
      ...
      <div className="mt-6 flex flex-wrap items-center gap-4">
        ...
        {state.status === "success" && <p className="text-sm text-emerald-400" role="status">{t.sent}</p>}
        {state.status === "error" && <p className="text-sm text-red-400" role="alert">{t.failed}</p>}
      </div>
    </form>
  );
}
