...
export function SkillForm({ skill }: { skill?: Skill }) {
  ...
  return <form action={action} className="grid gap-4 sm:grid-cols-[1fr_1fr_8rem_auto] sm:items-end">
    <FormToast state={state} />
    {skill && <input type="hidden" name="id" value={skill.id} />}
    <input type="hidden" name="iconUrl" value={skill?.iconUrl ?? ""} />
    <AdminInput label="مهارت" name="name" defaultValue={skill?.name} required />
    <AdminInput label="لوگوی فناوری" name="iconFile" type="file" accept="image/jpeg,image/png,image/webp" />
    <AdminInput label="ترتیب" name="sortOrder" type="number" min="0" defaultValue={skill?.sortOrder ?? 0} required />
